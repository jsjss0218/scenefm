...
// Optional Vercel env vars: SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET.
...
async function getAppToken() {
  ...
  if (!upstream.ok) throw new Error(`Spotify 앱 토큰 발급 실패 (${upstream.status})`);
...
async function spotifyGet(path, userToken = "", attempt = 0) {
  ...
  const upstream = await fetch("https://api.spotify.com/v1" + path, {
  ...
  if (!upstream.ok) {
    ...
    throw new Error(`Spotify ${upstream.status}${detail ? `: ${detail}` : ""}`);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    sendJson(res, 200, { results, uris, matched: uris.length, total: tracks.length });
  } catch (e) {
    sendJson(res, 502, { error: e.message || "Spotify 곡 매칭에 실패했어요." });
  }
}
